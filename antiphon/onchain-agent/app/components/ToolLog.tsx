...
export function ToolLog({ events, agentSlug, compact }: Props) {
  const [expanded, setExpanded] = useState(!compact);

  if (events.length === 0) return null;

  const pending = events.filter((e) => e.status === "pending").length;
  const label = agentSlug ?? "agent";

  return (
    <div className={`w-full ${compact ? "" : "max-w-2xl mt-2"}`}>
      <button
        type="button"
            ...
            {events.map((e, i) => (
              ...
              >
                <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                  {e.status === "pending" ? (
                    <span className="text-amber-400/70 shrink-0 animate-pulse text-xs">
                      ⟳
                    </span>
                  ) : (
                  ...
                  {e.durationMs != null && e.status === "done" && (
                    <span className="text-[10px] text-[#64748b]">{e.durationMs}ms</span>
                  )}
                </div>
