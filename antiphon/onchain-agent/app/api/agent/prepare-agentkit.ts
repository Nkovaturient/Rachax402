 ...
 *
 * Network: set NETWORK_ID to `base-sepolia` (default) or `base-mainnet`.
 * RPC_URL or BASE_RPC_URL overrides the default public RPC for reads and userOp simulation.
 *
 ...
 *   2. CDP facilitator calls Permit2.permitWitnessTransferFrom() on the same chain as NETWORK_ID.
 ...
 * 
 *  * Therefore the ONE-TIME prerequisite is USDC.approve(Permit2, MaxUint256) FROM the smart wallet
 * as an ERC-4337 user operation (ensurePermit2Approval).
 * ────────────────────────────────────────────────────────────────────────────────
 *
...
async function ensurePermit2Approval(
  ...
  const chain = viemChainForCdpNetwork(networkId);
  const rpcUrl = rpcUrlForViem(networkId);
  const usdc = usdcAddressForCdpNetwork(networkId);
  const explorer = blockExplorerOrigin(networkId);
  ...
  if (allowance > 0n) {
    console.log(
      `[Permit2] ✅ Allowance already set (${allowance.toString()} wei) — skipping approval.`
    );
    return;
  }

  console.log(
    "[Permit2] Allowance is 0 — sending USDC.approve(Permit2, MaxUint256) as userOp…"
  );
  console.log(`[Permit2]   Network      : ${networkId}`);
  console.log(`[Permit2]   Smart wallet : ${smartWalletAddress}`);
  console.log(`[Permit2]   USDC         : ${usdc}`);
  ...
  try {
    ...
    console.log(`[Permit2] Explorer: ${explorer}/address/${smartWalletAddress}`);
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
    console.warn(
      `\n[Permit2] ⚠️  Could not send USDC approval userOp: ${msg}\n` +
        ...
        `[Permit2] Or approve manually at: ${explorer}/address/${smartWalletAddress}\n`
    );
    return;
  }
...
export async function prepareAgentkitAndWalletProvider(): Promise<{
  ...
  try {
    ...
    await ensurePermit2Approval(walletProvider, networkId);
    ...
    console.log(`[AgentKit] Ready on ${networkId} (chain ${viemChainForCdpNetwork(networkId).id})`);
